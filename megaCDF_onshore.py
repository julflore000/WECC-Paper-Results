...
# create cdf
d = Dataset('wecc_results_onshore.nc','w')
...
def extract_map(results_filename):
    ...
    onshore_map = np.where(onshore_bounds == 0, elcc_map, np.nan)
    
    #### can graph the two together to see the removal of offshore data points if need be
    ''' sns.heatmap(elcc_map.T)
    plt.show()
    sns.heatmap(onshore_map.T)
    plt.show()'''
    
    elcc_map = onshore_map
    return elcc_map
